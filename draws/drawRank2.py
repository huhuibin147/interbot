...
def drawR(mapjson, rankjson, userjson):
    ...
    if me_idx != -1:
        ...
        d.add_text(120, 590, '%s'%(udict[u]), font_size=25, ttype='en')
    ...
    for i in range(r2):
        ...
        d.add_text(720, 160+i*offset1, '%s'%(udict[u]), font_size=25, ttype='en')
...
def r(bg):
    im = d.get_img('', path='../image/bg/%s'%bg)
    im = im.resize((1366, 768))
    # 起始点的横坐标，起始点的纵坐标，宽度，高度
    im2 = im.crop((0,0,1366,768//2))
    im2 = im2.resize((1366//2,768//2//2))
    return im2

def add_text(img, x, y, text, font_size=28, color='white', font_path=None, ttype='en'):
    f = 'font/msyh.ttc'
    font = ImageFont.truetype(f, font_size)
    ImageDraw.Draw(img).text((x, y), text, font=font, fill=color)

d = DrawRec()
target = Image.new('RGB',(1366,768))
bg = 'newgame_background.png' 
bg2 = '1518000.jpg' 
bg3 = '1025946.jpg' 
bg4 = '814293.jpg' 
im2 = r(bg)
...
st = 768//2//2
offset = -35
target.paste(im2, (0, 0*st))
target.paste(im3, (0, 1*st))
target.paste(im4, (0, 2*st))
target.paste(im5, (0, 3*st))
add_text(target, 5, 0*st, '%s (%s) - %s [%s]'%('ちいさな彼女の小夜曲','Sasaki Sayaka','Kiss no Hitotsu de','Garden\'s Extra'), color='black', font_size=15)
add_text(target, 5, 1*st, '%s (%s) - %s [%s]'%('ちいさな彼女の小夜曲','Sasaki Sayaka','Kiss no Hitotsu de','Garden\'s Extra'),color='black',  font_size=15)
add_text(target, 5, 2*st, '%s (%s) - %s [%s]'%('ちいさな彼女の小夜曲','Sasaki Sayaka','Kiss no Hitotsu de','Garden\'s Extra'),color='black',  font_size=15)
add_text(target, 5, 3*st, '%s (%s) - %s [%s]'%('ちいさな彼女の小夜曲','Sasaki Sayaka','Kiss no Hitotsu de','Garden\'s Extra'),color='black',  font_size=15)
add_text(target, 5, 4*st, '%s (%s) - %s [%s]'%('ちいさな彼女の小夜曲','Sasaki Sayaka','Kiss no Hitotsu de','Garden\'s Extra'),color='black',  font_size=15)

add_text(target, 900, 10, '第一期Chart',font_size=30)
add_text(target, 700, 70, '规则说明: 打爆这4张图打爆这4张图',font_size=30)
add_text(target, 700, 110, '规则说明: 打爆这4张图打爆这4张图',font_size=30)
add_text(target, 700, 150, '规则说明: 打爆这4张图打爆这4张图',font_size=30)
add_text(target, 700, 250, '奖励: int妹妹女装int妹妹女装',font_size=30, color='yellow')
add_text(target, 700, 290, '奖励: int妹妹女装int妹妹女装',font_size=30, color='yellow')
add_text(target, 700, 330, '奖励: int妹妹女装int妹妹女装',font_size=30, color='yellow')
add_text(target, 5, 50+offset, 'mapped by %s'%('val0108'), font_size=15, ttype='cn',color='black')
add_text(target, 5, 70+offset, '长度: %s  BPM: %s  物件数: %s'%('03:20',198,1020), font_size=15, ttype='cn',color='red')
add_text(target, 5, 90+offset, 'CS:%s AR:%s OD:%s HP:%s Stars:%s'%(4,9,8,8,4.71), font_size=14, ttype='en',color='red')

add_text(target, 5, 50+1*st+offset, 'mapped by %s'%('val0108'), font_size=15, ttype='cn',color='black')
add_text(target, 5, 70+1*st+offset, '长度: %s  BPM: %s  物件数: %s'%('03:20',198,1020), font_size=15, ttype='cn',color='red')
add_text(target, 5, 90+1*st+offset, 'CS:%s AR:%s OD:%s HP:%s Stars:%s'%(4,9,8,8,4.71), font_size=14, ttype='en',color='red')

add_text(target, 5, 50+2*st+offset, 'mapped by %s'%('val0108'), font_size=15, ttype='cn',color='black')
add_text(target, 5, 70+2*st+offset, '长度: %s  BPM: %s  物件数: %s'%('03:20',198,1020), font_size=15, ttype='cn',color='red')
add_text(target, 5, 90+2*st+offset, 'CS:%s AR:%s OD:%s HP:%s Stars:%s'%(4,9,8,8,4.71), font_size=14, ttype='en',color='red')

add_text(target, 5, 50+3*st+offset, 'mapped by %s'%('val0108'), font_size=15, ttype='cn',color='black')
add_text(target, 5, 70+3*st+offset, '长度: %s  BPM: %s  物件数: %s'%('03:20',198,1020), font_size=15, ttype='cn',color='red')
add_text(target, 5, 90+3*st+offset, 'CS:%s AR:%s OD:%s HP:%s Stars:%s'%(4,9,8,8,4.71), font_size=14, ttype='en',color='red')
target.show()
# d.RecImg.show()
